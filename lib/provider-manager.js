...
const { Selector } = require("@lumine-code/selector-kit");
...
module.exports = class ProviderManager {
  ...
  initialize() {
    ...
    this.subscriptions.add(
      lumine.config.observe("autocomplete.enableBuiltinProvider", (value) =>
        this.toggleDefaultProvider(value),
      ),
    );
    this.subscriptions.add(
      lumine.config.observe("autocomplete.scopeBlacklist", (value) =>
  ...
  registerProvider(provider) {
    ...
    if (validationErrors.length > 0) {
      ...
      lumine.notifications.addError(`Invalid autocomplete provider rejected: ${name}`, {
        description:
