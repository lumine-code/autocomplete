...
const createSuggestionFrag = (document) => {
...
module.exports = class SuggestionListElement {
  constructor(model, document = globalThis.document) {
  ...
  adoptDocument(document) {
    if (this.element.ownerDocument !== document) document.adoptNode(this.element);
  }
  ...
  renderItems() {
    ...
    return lumine.views.updateDocument(this.returnItemsToPool.bind(this, items.length));
  }
  ...
  renderExtraItems() {
    ...
    if (this.extraItems.length) {
      this.extraItemsFrameWindow = this.element.ownerDocument.defaultView;
      this.extraItemsFrame = this.extraItemsFrameWindow.requestAnimationFrame(() => {
        this.extraItemsFrame = null;
        lumine.views.updateDocument(this.renderExtraItems.bind(this));
      });
    } else {
  ...
  cancelExtraItems() {
    if (this.extraItemsFrame != null) {
      this.extraItemsFrameWindow?.cancelAnimationFrame(this.extraItemsFrame);
      this.extraItemsFrame = null;
      this.extraItemsFrameWindow = null;
    }
    this.extraItems = null;
  }
  ...
  readUIPropsFromDOM() {
    ...
    if (!this.uiProps.paddingHeight) {
      this.uiProps.paddingHeight =
        parseInt(
          this.element.ownerDocument.defaultView.getComputedStyle(this.element)["padding-top"],
        ) +
        parseInt(
          this.element.ownerDocument.defaultView.getComputedStyle(this.element)["padding-bottom"],
        );
      if (!this.uiProps.paddingHeight) {
  ...
  renderItem(
    ...
    if (!li) {
      if (this.nodePool && this.nodePool.length > 0) {
        ...
        li = this.element.ownerDocument.createElement("li");
        li.appendChild(createSuggestionFrag(this.element.ownerDocument));
      }
    ...
    const iconContainer = this.element.ownerDocument.createElement("span");
    ...
    if ((sanitizedIconHTML || sanitizedType) && iconHTML !== false) {
      let icon = this.element.ownerDocument.createElement("i");
      icon.className = "icon";
      ...
      if (sanitizedIconHTML != null) {
        ...
        if (Object.hasOwn(iconTypeToClass, sanitizedType)) {
          defaultIcon = this.element.ownerDocument.createElement("i");
          defaultIcon.className = iconTypeToClass[sanitizedType];
        } else if (sanitizedType) {
          defaultIcon = this.element.ownerDocument.createElement("span");
    ...
    const wordSpan = this.element.ownerDocument.createElement("span");
  ...
  getDisplayFragment(text, snippet, displayText, replacementPrefix, characterMatchIndices) {
    ...
    const appendNonMatchChars = (el, nonMatchChars) => {
      if (nonMatchChars) {
        el.appendChild(this.element.ownerDocument.createTextNode(nonMatchChars));
      }
    };
    ...
    for (let index = 0; index < replacementText.length; index++) {
      if (
        ...
        let s = this.element.ownerDocument.createElement("span");
