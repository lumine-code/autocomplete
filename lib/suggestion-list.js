...
module.exports = class SuggestionList {
  ...
  initialize() {
    ...
    this.subscriptions.add(
      lumine.commands.add("lumine-text-editor.autocomplete-active", {
        "autocomplete:confirm": {
          description: "Insert the suggestion now highlighted.",
          didDispatch: this.confirmSelection,
        },
        "autocomplete:confirmIfNonDefault": {
          description: "Insert the suggestion, unless it is the one already there.",
          didDispatch: this.confirmSelectionIfNonDefault,
        },
        "autocomplete:cancel": {
          description: "Close the suggestion list without taking anything.",
          didDispatch: this.cancel,
        },
      }),
    );
    this.subscriptions.add(
    ...
    this.subscriptions.add(
      lumine.workspace.addWindowSurfaceTransitionObserver((context) => {
        if (context.item === this.activeEditor) this.hide();
      }),
    );
  }
  ...
  */
  ...
  show(editor, options) {
    ...
    if (editorElement) this.suggestionListElement.adoptDocument(editorElement.ownerDocument);
